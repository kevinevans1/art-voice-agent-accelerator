...
@pytest.fixture
async def media_handler(mock_websocket, mock_recognizer, mock_orchestrator, mock_memory_manager):
...
class TestMainEventLoop:
    ...
    async def test_handle_audio_metadata(self, main_event_loop, mock_recognizer):
        ...
        await main_event_loop.handle_media_message(stream_data, mock_recognizer, acs_handler)
    ...
    async def test_handle_audio_data(self, main_event_loop, mock_recognizer):
        ...
        with patch.object(main_event_loop, '_process_audio_chunk_async') as mock_process:
            await main_event_loop.handle_media_message(stream_data, mock_recognizer, None)
...
class TestRouteTurnThread:
    ...
    @pytest.mark.asyncio
    async def test_initialization(self, mock_orchestrator, mock_memory_manager, mock_websocket):
    ...
    @pytest.mark.asyncio
    async def test_speech_event_processing(self, mock_orchestrator, mock_memory_manager, mock_websocket):
...
class TestACSMediaHandler:
    ...
    async def test_media_message_processing(self, mock_logger, media_handler, mock_recognizer):
    ...
    @pytest.mark.asyncio
    @patch('apps.rtagent.backend.api.v1.handlers.acs_media_lifecycle.logger')
    async def test_barge_in_flow(self, mock_logger, media_handler, mock_recognizer, mock_orchestrator):
        """Test complete barge-in detection and cancellation flow."""
        # Start processing by triggering recognizer
        await media_handler.handle_media_message(json.dumps({
            "kind": "AudioMetadata",
            "audioMetadata": {"subscriptionId": "test"}
        }))
    ...
    @pytest.mark.asyncio
    @patch('apps.rtagent.backend.api.v1.handlers.acs_media_lifecycle.logger')
    async def test_speech_recognition_callbacks(self, mock_logger, media_handler, mock_recognizer, mock_orchestrator):
    ...
    @pytest.mark.asyncio
    @patch('apps.rtagent.backend.api.v1.handlers.acs_media_lifecycle.logger')
    async def test_concurrent_audio_processing(self, mock_logger, media_handler, mock_recognizer):
...
class TestIntegrationScenarios:
    ...
    @pytest.mark.asyncio
    @patch('apps.rtagent.backend.api.v1.handlers.acs_media_lifecycle.logger')
    async def test_call_flow_with_greeting(self, mock_logger, mock_websocket, mock_recognizer, mock_orchestrator, mock_memory_manager):
        ...
        try:
            ...
            await handler.handle_media_message(json.dumps({
                "kind": "AudioMetadata",
                "audioMetadata": {
            ...
            assert len(mock_orchestrator.calls) >= 1
            assert any("account" in call['transcript'].lower() for call in mock_orchestrator.calls)
    ...
    @pytest.mark.asyncio
    @patch('apps.rtagent.backend.api.v1.handlers.acs_media_lifecycle.logger')
    async def test_barge_in_during_response(self, mock_logger, mock_websocket, mock_recognizer, mock_orchestrator, mock_memory_manager):
