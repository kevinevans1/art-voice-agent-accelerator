...
async def test_cancel_call_for_validation_failure_with_session_terminator(mock_context):
    ...
    # Act
    with patch('apps.rtagent.backend.api.v1.handlers.dtmf_validation_lifecycle.terminate_session', new_callable=AsyncMock) as mock_terminate:
...
async def test_public_cancel_method():
    ...
    with patch.object(DTMFValidationLifecycle, '_cancel_call_for_validation_failure') as mock_private:
