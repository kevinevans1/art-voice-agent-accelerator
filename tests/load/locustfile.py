...
TURN_TIMEOUT_SEC = float(os.getenv("TURN_TIMEOUT_SEC", "12.0"))
    ...
    def speech_turns(self):
        for _ in range(TURNS_PER_USER):
            ...
            try:
                ...
                self._record(name=f"barge_in[{Path(file_used).name}->{Path(next_file_used).name}]", response_time_ms=barge_ms, exc=None if barge_ok else Exception("barge_end_timeout"))
