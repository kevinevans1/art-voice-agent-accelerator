...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('/tmp/azure_load_test.log') if '/tmp' in os.environ.get('PATH', '') else logging.NullHandler()
    ]
)
...
class AzureLoadTestConfig:
    ...
    conversation_templates: List[str] = field(default_factory=lambda: ["quick_question", "insurance_inquiry"])
    ws_url: str = "ws://localhost:8010/api/v1/media/stream"
    
    # Azure-specific parameters
    azure_test_id: str = field(default_factory=lambda: f"load-test-{uuid.uuid4().hex[:8]}")
    ...
    def calculate_summary_stats(self) -> Dict[str, Any]:
        ...
        if self.response_times_ms:
            summary["response_times_ms"] = {
                ...
                "p90": statistics.quantiles(self.response_times_ms, n=10)[8] if len(self.response_times_ms) >= 10 else max(self.response_times_ms),
                "p95": statistics.quantiles(self.response_times_ms, n=20)[18] if len(self.response_times_ms) >= 20 else max(self.response_times_ms),
                "p99": statistics.quantiles(self.response_times_ms, n=100)[98] if len(self.response_times_ms) >= 100 else max(self.response_times_ms),
                "std_dev": statistics.stdev(self.response_times_ms) if len(self.response_times_ms) > 1 else 0,
            }
        ...
        if self.agent_response_times_ms:
            summary["agent_performance_ms"] = {
                ...
                "p95_response_time": statistics.quantiles(self.agent_response_times_ms, n=20)[18] if len(self.agent_response_times_ms) >= 20 else max(self.agent_response_times_ms),
        ...
        summary["conversations"] = {
            ...
            "completion_rate_percent": (self.conversations_completed / max(1, self.conversations_completed + self.conversations_failed)) * 100,
...
class AzureConversationLoadTester:
    ...
    def log_azure_metric(self, metric_name: str, value: float, dimensions: Dict[str, str] = None):
    ...
    async def run_single_conversation_azure(
        ...
        async with semaphore:
            ...
            try:
                logger.info(f"Starting Azure conversation {user_id} with template {template.name}")
                
                # Log conversation start
                self.log_azure_metric("conversation_started", 1, {
                ...
                if metrics.total_agent_processing_time_ms > 0:
                    ...
                    self.log_azure_metric("agent_response_time_ms", avg_agent_time, {
                        "template": template.name,
                        "user_id": user_id
                    })
                ...
                if metrics.total_speech_recognition_time_ms > 0:
                    ...
                    self.log_azure_metric("speech_recognition_time_ms", avg_speech_time, {
                        "template": template.name,
                        "user_id": user_id
                    })
                ...
                self.log_azure_metric("conversation_completed", 1, {
                    "template": template.name,
                    "user_id": user_id,
                ...
                logger.info(f"Completed Azure conversation {user_id} in {conversation_duration:.2f}s")
                return metrics
                ...
                # Log error to Azure
                self.log_azure_metric("conversation_failed", 1, {
                    "template": template.name,
                    "user_id": user_id,
    ...
    async def run_azure_load_test_scenario(self, scenario_name: str) -> Dict[str, Any]:
        ...
        scenario_configs = {
            ...
            "heavy": {
                ...
                "duration_minutes": 10,
                "target_rps": 10.0,
                "templates": ["quick_question", "insurance_inquiry", "confused_customer"]
            },
            "stress": {
                "max_concurrent": 100,
                "duration_minutes": 15,
        ...
        logger.info(f"⏰ Test duration: {test_duration_s}s, Target conversations: {total_conversations}")
        ...
        try:
            while time.time() - test_start_time < test_duration_s and conversation_counter < total_conversations:
                elapsed_time = time.time() - test_start_time
                
                # Calculate current RPS based on ramp-up
                if elapsed_time < ramp_up_duration_s:
                    current_rps = (elapsed_time / ramp_up_duration_s) * self.config.target_rps
                else:
                ...
                if len(active_tasks) < self.config.max_concurrent_users and conversation_counter < total_conversations:
                    template_name = self.config.conversation_templates[conversation_counter % len(self.config.conversation_templates)]
                ...
                if int(elapsed_time) % 30 == 0 and elapsed_time > 0:
                    logger.info(f"⏱️  Progress: {elapsed_time:.0f}s, Active: {len(active_tasks)}, "
            ...
            if active_tasks:
                logger.info(f"⏳ Waiting for {len(active_tasks)} remaining conversations...")
            ...
            duration_s = (self.metrics.end_time - self.metrics.start_time).total_seconds()
            self.metrics.requests_per_second = self.metrics.total_requests / max(duration_s, 1)
            self.metrics.error_rate_percent = (self.metrics.failed_requests / max(self.metrics.total_requests, 1)) * 100
            
            if self.metrics.response_times_ms:
                self.metrics.avg_conversation_duration_s = statistics.mean(self.metrics.response_times_ms) / 1000
        
        # Log final metrics
        self.log_azure_metric("test_completed", 1, {
            "scenario": scenario_name,
            "total_requests": str(self.metrics.total_requests),
            "success_rate": str((self.metrics.successful_requests / max(self.metrics.total_requests, 1)) * 100)
        })
    ...
    def _calculate_aggregate_metrics(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        total_requests = sum(r["summary"]["performance"]["total_requests"] for r in results)
        total_successful = sum(r["summary"]["performance"]["successful_requests"] for r in results)
        total_failed = sum(r["summary"]["performance"]["failed_requests"] for r in results)
        ...
        for result in results:
            if "response_times_ms" in result["summary"]:
                ...
                all_response_times.extend([result["summary"]["response_times_ms"]["mean"]])
            
            if "agent_performance_ms" in result["summary"]:
                all_agent_times.extend([result["summary"]["agent_performance_ms"]["mean_response_time"]])
        ...
        aggregate = {
            "overall_performance": {
                ...
                "overall_success_rate_percent": (total_successful / max(total_requests, 1)) * 100,
...
async def main():
    ...
    config = AzureLoadTestConfig(
        ...
        ws_url=os.environ.get('WEBSOCKET_URL', 'ws://localhost:8010/api/v1/media/stream'),
        azure_test_id=os.environ.get('AZURE_TEST_ID', f'load-test-{uuid.uuid4().hex[:8]}'),
    ...
    for i, scenario in enumerate(scenarios):
        try:
            ...
            results.append({
                "scenario": scenario,
                "error": str(e),
                "summary": {"performance": {"total_requests": 0, "successful_requests": 0, "failed_requests": 1}}
            })
    ...
    print(f"📊 Scenarios completed: {len([r for r in results if 'error' not in r])}/{len(scenarios)}")
    print(f"💾 Results saved to: {results_file}")
    
    # Log final summary for Azure ingestion
    total_requests = sum(r.get("summary", {}).get("performance", {}).get("total_requests", 0) for r in results)
    total_success = sum(r.get("summary", {}).get("performance", {}).get("successful_requests", 0) for r in results)
    
    logger.info(f"AZURE_SUMMARY: Total requests: {total_requests}, Successful: {total_success}, Success rate: {(total_success/max(total_requests,1))*100:.1f}%")
