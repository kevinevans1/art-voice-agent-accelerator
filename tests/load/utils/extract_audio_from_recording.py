        ...
        for conv_idx, conversation in enumerate(conversations):
            ...
            for turn in conversation.get("turns", []):
                ...
                if "full_responses_received" in turn and turn["full_responses_received"]:
                    ...
                    audio_chunks = self.extract_audio_from_responses(turn["full_responses_received"])
                    ...
                    if audio_chunks:
                        ...
                        if combined_audio and self.speech_enabled:
                            print(f"    🔄 Transcribing combined audio ({len(combined_audio)} bytes)...")
                            ...
                            print(f"    📄 Audio extracted but speech recognition not available")
                            turn_result["combined_audio_text"] = "[Audio available - speech recognition disabled]"
        ...
        for conv in results.get("conversations", []):
            ...
            for turn in conv["turns"]:
                ...
                if turn["combined_audio_text"]:
                    ...
                    print(f"    Agent: [Found {turn['audio_chunks_found']} audio chunks but no text transcribed]")
                else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Extract and transcribe audio from conversation recordings")
    parser.add_argument("--file", "-f", required=True,
                       help="Path to conversation recording JSON file")
