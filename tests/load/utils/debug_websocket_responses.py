...
class WebSocketResponseDebugger:
    ...
    async def debug_single_turn(self, websocket_url: str = "ws://localhost:8000/ws"):
        ...
        async with websockets.connect(websocket_url) as websocket:
            ...
            while time.time() - start_time < 15.0:
                try:
                    ...
                    if response_types[response_kind] <= 3:
                        ...
                        print(f"📨 {response_kind}: (continuing to receive, stopping detailed logs...)")
            ...
            speech_responses = [r for r in responses if 'speech' in r.get('kind', '').lower() or 
                             'recognition' in r.get('kind', '').lower() or
                             'text' in str(r).lower()]
            ...
            text_responses = [r for r in responses if 'text' in r.get('kind', '').lower() or 
                            'message' in r.get('kind', '').lower() or
                            any(key in r for key in ['text', 'message', 'content'])]
...
async def main():
    ...
    try:
        ...
        print(f"📄 Use this information to update conversation_simulator.py response parsing")
