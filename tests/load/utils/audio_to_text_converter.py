        ...
        for conv_idx, conversation in enumerate(conversations):
            ...
            for turn in conversation["turns"]:
                ...
                if "audio_files" in turn and turn["audio_files"]:
                    print(f"  🔄 Turn {turn['turn_number']}: Transcribing {len(turn['audio_files'])} audio files...")
                        ...
                        if "path" in audio_file_info:
                            ...
                            if transcription.success:
                                if transcription.transcribed_text.strip():
                                    print(f"    ✅ {audio_file_info.get('filename', 'audio')}: '{transcription.transcribed_text}'")
                                    results["transcription_summary"]["successfully_transcribed"] += 1
                                else:
                                    print(f"    📭 {audio_file_info.get('filename', 'audio')}: No speech detected")
                                    results["transcription_summary"]["empty_audio"] += 1
                            else:
                                print(f"    ❌ {audio_file_info.get('filename', 'audio')}: {transcription.error_message}")
