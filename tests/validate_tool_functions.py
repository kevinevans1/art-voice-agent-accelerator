...
def validate_tool_registry():
    ...
    try:
        ...
        from agents.tool_store.tool_registry import function_mapping, available_tools, TOOL_REGISTRY
    ...
    for func_name, func in function_mapping.items():
        ...
        tool_found = any(tool['function']['name'] == func_name for tool in available_tools)
        if tool_found:
    ...
    orphaned_schemas = [name for name in schemas.keys() if name not in function_mapping.keys()]
    if orphaned_schemas:
    ...
    missing_schemas = [name for name in function_mapping.keys() if name not in schemas.keys()]
    if missing_schemas:
