...
class HealthResponse(BaseModel):
    ...
    timestamp: float = Field(..., description="Timestamp when check was performed", example=1691668800.0)
    message: str = Field(..., description="Human-readable status message", example="Real-Time Audio Agent API v1 is running")
...
class ServiceCheck(BaseModel):
    ...
    component: str = Field(..., description="Name of the component being checked", example="redis")
    status: str = Field(
        ..., 
    ...
    check_time_ms: float = Field(..., description="Time taken to perform the check in milliseconds", example=12.5)
    error: Optional[str] = Field(None, description="Error message if check failed", example="Connection timeout")
    details: Optional[str] = Field(None, description="Additional details about the check", example="Connected to Redis successfully")
...
class ReadinessResponse(BaseModel):
    ...
    timestamp: float = Field(..., description="Timestamp when check was performed", example=1691668800.0)
    response_time_ms: float = Field(..., description="Total time taken for all checks in milliseconds", example=45.2)
