...
class WebhookBatchResponse(BaseModel):
    ...
    batch_id: Optional[str] = Field(None, description="Batch identifier", example="batch-123")
    total_events: int = Field(..., description="Total number of events in batch", example=5)
    successful_events: int = Field(..., description="Number of successfully processed events", example=4)
    failed_events: int = Field(..., description="Number of failed events", example=1)
    processing_time_ms: int = Field(..., description="Total processing time in milliseconds", example=150)
