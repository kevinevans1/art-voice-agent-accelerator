...
class Event(TimestampedModel):
    ...
    participant_id: Optional[str] = Field(
        default=None,
        description="Associated participant ID if applicable"
    )
...
class EventHistory(TimestampedModel):
    ...
    attempt_number: int = Field(
        description="Processing attempt number (1-based)"
    )
    
    status: EventStatus = Field(
        description="Status of this processing attempt"
    )
    ...
    handler_version: Optional[str] = Field(
        default=None,
        description="Version of the event handler"
    )
