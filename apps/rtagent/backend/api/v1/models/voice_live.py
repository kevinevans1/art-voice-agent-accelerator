...
class VoiceLiveAudioConfig(BaseModel):
    ...
    sample_rate: int = Field(default=24000, description="Audio sample rate in Hz", json_schema_extra={"enum": [16000, 24000]})
    ...
    vad_mode: VoiceActivityDetectionMode = Field(default=VoiceActivityDetectionMode.AUTO, description="VAD mode")
    vad_sensitivity: float = Field(default=0.3, description="VAD threshold (0.0-1.0)", ge=0.0, le=1.0)
    ...
    echo_cancellation: bool = Field(default=True, description="Enable server-side echo cancellation")
    ...
    input_audio_noise_reduction_type: str = Field(
        ...
        description="Azure noise reduction type"
    )
    input_audio_echo_cancellation_type: str = Field(
        default="server_echo_cancellation",
        description="Azure echo cancellation type"
    )
...
class VoiceLiveModelConfig(BaseModel):
    ...
    voice_name: str = Field(default="en-US-Ava:DragonHDLatestNeural", description="Azure neural voice name")
    ...
    voice_temperature: float = Field(default=0.8, description="Voice temperature for HD voices", ge=0.0, le=1.0)
    ...
    api_version: str = Field(default="2025-05-01-preview", description="Azure Voice Live API version")
    ...
    silence_duration_ms: int = Field(default=200, description="Silence duration in milliseconds", ge=0)
    remove_filler_words: bool = Field(default=False, description="Remove filler words")
    
    # End of utterance detection
    end_of_utterance_enabled: bool = Field(default=True, description="Enable end of utterance detection")
    end_of_utterance_model: str = Field(default="semantic_detection_v1", description="End of utterance model")
    end_of_utterance_threshold: float = Field(default=0.01, description="End of utterance threshold", ge=0.0, le=1.0)
    end_of_utterance_timeout: int = Field(default=2, description="End of utterance timeout in seconds", ge=1)
...
class VoiceLiveMetrics(BaseModel):
    ...
    session_id: str = Field(..., description="Session identifier")
    measurement_start: datetime = Field(default_factory=datetime.utcnow, description="Measurement start time")
    last_update: datetime = Field(default_factory=datetime.utcnow, description="Last metrics update")
    
    # Latency Metrics (in milliseconds)
    audio_to_text_latency: Optional[float] = Field(None, description="Audio to text latency", ge=0.0)
    text_to_response_latency: Optional[float] = Field(None, description="Text to response latency", ge=0.0)
    response_to_audio_latency: Optional[float] = Field(None, description="Response to audio latency", ge=0.0)
    end_to_end_latency: Optional[float] = Field(None, description="End-to-end latency", ge=0.0)
    
    # Quality Metrics
    speech_recognition_confidence: Optional[float] = Field(None, description="Speech recognition confidence", ge=0.0, le=1.0)
    audio_quality_score: Optional[float] = Field(None, description="Audio quality score", ge=0.0, le=1.0)
    voice_activity_accuracy: Optional[float] = Field(None, description="VAD accuracy", ge=0.0, le=1.0)
    
    # Resource Metrics
    cpu_usage_percent: Optional[float] = Field(None, description="CPU usage percentage", ge=0.0, le=100.0)
    memory_usage_mb: Optional[float] = Field(None, description="Memory usage in MB", ge=0.0)
    network_throughput_kbps: Optional[float] = Field(None, description="Network throughput in kbps", ge=0.0)
...
class VoiceLiveSession(BaseModel):
    ...
    status: VoiceLiveSessionStatus = Field(default=VoiceLiveSessionStatus.INITIALIZING, description="Session status")
    status_message: Optional[str] = Field(None, description="Status message")
    
    # Timestamps
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Session creation time")
    connection_established_at: Optional[datetime] = Field(None, description="Connection establishment time")
    ...
    # Configuration
    audio_config: VoiceLiveAudioConfig = Field(default_factory=VoiceLiveAudioConfig, description="Audio configuration")
    model_configuration: VoiceLiveModelConfig = Field(default_factory=VoiceLiveModelConfig, description="Model configuration")
    
    # Connection State
    websocket_connected: bool = Field(default=False, description="WebSocket connection status")
    azure_speech_connected: bool = Field(default=False, description="Azure Speech connection status")
    ...
    # Performance Metrics
    average_response_time_ms: float = Field(default=0.0, description="Average response time", ge=0.0)
    ...
    def set_status(self, status: VoiceLiveSessionStatus, message: Optional[str] = None) -> None:
