...
class TurnProcessor:
            ...
            try:
                while not self.stopped:
                    try:
                        ...
                        op.log_info(f"Processing {kind} turn: {text[:50]}... (queue_wait: {queue_wait_time:.3f}s)")
    ...
    async def _route_and_playback(self, kind: str, text: str):
        ...
        with trace_acs_dependency(
            tracer, logger, "orchestration", "route_conversation_turn",
            call_connection_id=self.call_connection_id
        ) as dep_op:
            ...
            try:
                ...
                safe_set_span_attributes(dep_op, {
                ...
                dep_op.log_info(f"Orchestration completed successfully in {orchestration_duration:.3f}s")
...
class ACSMediaHandler:
    ...
    async def start_recognizer(self):
        ...
        with trace_acs_operation(
            ...
            op.log_info(f"Speech recognizer started in {recognizer_setup_duration:.3f}s")
            ...
            op.log_info(f"ACS Media Handler started successfully in {total_startup_time:.3f}s")
    ...
    async def _play_greeting(self):
        ...
        with tracer.start_as_current_span(
            ...
            try:
                ...
                logger.info(f"🎵 Greeting played in {greeting_send_duration:.3f}s: {self.greeting_text}")
