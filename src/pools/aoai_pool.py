...
class AOAIClientPool:
    ...
    def __init__(self, pool_size: int = None):
        ...
        logger.info(f"AOAI client pool initializing with {self.pool_size} clients (enabled={AOAI_POOL_ENABLED})")
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(f"AOAI client pool initialized successfully with {len(self.clients)} clients")
    ...
    async def get_dedicated_client(self, session_id: str) -> AzureOpenAI:
        ...
        with self.lock:
            ...
            if session_id in self.session_allocations:
                ...
                logger.debug(f"Session {session_id} using existing AOAI client {client_index}")
    ...
    def _find_best_client(self) -> int:
        ...
        for i, metrics in enumerate(self.client_metrics):
            ...
            active_sessions = sum(1 for idx in self.session_allocations.values() if idx == i)
            load_score = active_sessions + (metrics.avg_response_time / 1000)  # Convert ms to seconds
    ...
    async def release_client(self, session_id: str) -> None:
        ...
        with self.lock:
            if session_id in self.session_allocations:
                ...
                logger.info(f"AOAI client {client_index} released from session {session_id}")
    ...
    async def request_context(self, session_id: str):
        ...
        try:
            ...
            logger.error(f"AOAI request failed for session {session_id} on client {client_index}: {e}")
            raise
