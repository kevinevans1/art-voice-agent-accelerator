...
class LatencyTool:
    ...
    def __init__(self, cm):
        ...
        # 🚀 OPTIMIZATION: Track active timers to prevent start/stop mismatches
    ...
    def start(self, stage: str) -> None:
        # 🚀 OPTIMIZATION: Track timer state to prevent duplicate starts
    ...
    def stop(self, stage: str, redis_mgr, *, meta: Optional[Dict[str, Any]] = None) -> None:
        # 🚀 OPTIMIZATION: Check timer state before stopping
    ...
    def cleanup_timers(self) -> None:
        """🚀 OPTIMIZATION: Clean up active timers on session disconnect."""
        if self._active_timers:
            logger.debug(f"[PERF] Cleaning up {len(self._active_timers)} active timers: {self._active_timers}")
